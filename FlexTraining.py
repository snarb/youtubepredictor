...
TrainingData = namedtuple('TrainingData', ['data', 'lables', 'shift'])
...
def ExtractSequenes(listOfVideoViews, minPredDelta, maxPredDelta, minLen, maxLen, seqLen, daysToRemove  = 30):
    if (not minLen):
        minLen = 0
    ...
    for i in range(len(listOfVideoViews)):
        ...
        if (len(s) > minLen and len(s) < maxLen):
            curSourcde = s[daysToRemove:]
            data = rolling_window(curSourcde, window=seqLen)

            for lableShift in range(minPredDelta, maxPredDelta):
                lable = curSourcde[seqLen - 1 + lableShift::1]
                #trainingData = TrainingData(data[:-lablesShift], lables, lablesShift)
                datas.extend(data[:-lableShift])
                lables.extend(lable)
                lablesShifts.append(lableShift)

    datas = np.log(datas)
    lables = np.log(lables)


    datas_res = np.clip(datas, a_min=STABLE_DELTA, a_max=99999999999)
    lables_res = np.clip(lables, a_min=STABLE_DELTA, a_max=99999999999)

    return datas_res, lables_res, lablesShifts
...
def TrainModel(train_inputs, train_lables, test_inputs, test_lables, SEQUENCE_LENGTH, predDelta):
    # train_inputs, train_lables = shuffle(train_inputs, train_lables)

    # if (absNorms is not None):
    #     train_inputs /= absNorms

    lableShift = tf.placeholder("float", name="lablesShift")
    X = tf.placeholder("float", shape=(None, SEQUENCE_LENGTH), name="viewsHistoryInput")  # , COLUMNS_COUNT
    Y = tf.placeholder("float", shape=(None, 1), name="Outputs")

    W = tf.Variable(tf.ones([SEQUENCE_LENGTH, 1]), name="weight")  # COLUMNS_COUNT
    basePrediction = tf.matmul(X, W)


    lsW = tf.Variable(tf.ones([1, SEQUENCE_LENGTH]), name="lableShiftsWeights")
    predDeltaKoefs = lsW * lableShift

    res = tf.matmul(basePrediction, predDeltaKoefs)
    ...
    for step in range(TRAIN_STEPS):
        ...
        _, curLoss = sess.run([train, loss], feed_dict={X: inputs, Y: output, lableShift: predDelta})
    ...
    predictTest = sess.run(pred, feed_dict={X: test_inputs, lableShift: predDelta})
...
#     rseMean, rseVar, mapeMean, mapeVar = TrainModel(train_inputs, train_lables, test_inputs, test_lables, seqLength, lableShift)
#
...
def TestDeltaPerf(listOfTrainVideoViews, listOfTestVideoViews, seqLen, minPredDelta, maxPredDelta, minLimit, maxLimit):
    id = threading.current_thread()
    print("Hi from thread id: ", id)

    train_inputs, train_lables = ExtractSequenes(listOfTrainVideoViews, minPredDelta, maxPredDelta, minLimit, maxLimit, seqLen, daysToRemove = 30)
    test_inputs, test_lables = ExtractSequenes(listOfTestVideoViews, minPredDelta, maxPredDelta, minLimit, maxLimit, seqLen, daysToRemove = 30)

    rseMean, rseVar, mapeMean, mapeVar = TrainModel(train_inputs, train_lables, test_inputs, test_lables, seqLen, lableShift)

    print("Complited: seqLength = ", seqLen)
    return rseMean, rseVar, mapeMean, mapeVar
...
res = TestDeltaPerf(trainViewsD, testViewsD, seqLen =  26, minPredDelta = 1, maxPredDelta = 130, minLimit = 130, maxLimit = None)
